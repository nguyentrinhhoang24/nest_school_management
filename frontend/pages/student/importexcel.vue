<template>
    <div>
        ...
        <form @submit.prevent="handleSubmit">
            ...
            <button @click="uploadStudent">Upload student</button>
        </form>
...
import { readStudentExcel } from '@/utils/ReadFileStudent';
...
const getBranchs = async () => {
    try {
        ...
        branchs.value = res || [];
        console.log('fetch branch in student:', branchs.value);
    } catch (error) {
...
const getClassByBranch = async (branch_id) => {
    try {
        ...
        classes.value = res || [];
        console.log('fetch class in student:', classes.value);
    } catch (error) {
...
const handleFileUpload = async (event) => {
    ...
    reader.onload = async (e) => {
        try {
            const data = await readStudentExcel(e.target.result);
...
const uploadStudent = async () => {
    try {
        const res = await $fetch(`http://localhost:5000/student/importexcel`, {
            method: 'POST',
            body: students.value,
        });
        console.log('response students:', res);
        alert('add students successfully');
        router.push('/student');
    } catch (error) {
        error.value = 'faild to upload students';
        console.error('error upload students:', error);
    }
}
