<template>
    <div class="menu-page">
        <h1>List menu
            <nuxt-link to="/menu/createmenu">+ Add new</nuxt-link>
        </h1>
        <div class="branch">
            ...
            <table>
                ...
                <tbody>
                    <tr v-for="item in menu" :key="item.id">
                        ...
                        <td>{{ item.status }}</td>
                        <td>
                            <nuxt-link :to="`/menu/updatemenu/${item._id}`" class="edit-button">Edit</nuxt-link>
...
const getMenu = async (branch_id) => {
    try {
        const token = localStorage.getItem('token');
        const { data } = await useFetch(`http://localhost:5000/menu/branchid/${branch_id}`);
        menu.value = data.value || [];
        console.log('fetch menu: ', menu.value);
    } catch (error) {
        console.log('catch fetch menu:', error)
    }
}

const remove = async (id) => {
    try {
        await useFetch(`http://localhost:5000/menu/${id}`, {
            method: 'DELETE',
        });
