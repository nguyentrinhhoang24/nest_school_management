<template>
    <div class="tag-page">
        <h1>List tag
            <nuxt-link to="/tag/createtag">+ Add New</nuxt-link>
        </h1>
        <div class="branch">
        ...
        <div>
            <table>
                ...
                <tbody>
                    <tr v-for="item in tag" :key="item.id">
                        <td>{{ item.title }}</td>
                        <td>{{ item.description }}</td>
                        <td>
                            <nuxt-link :to="`/tag/updatetag/${item._id}`" class="edit-button">Edit</nuxt-link>
...
const getTags = async (branch_id) => {
    try {
        ...
        console.log('fetch tag:', tag.value);
    } catch (error) {
        console.error('Catch fetching tag:', error);
    }
};
